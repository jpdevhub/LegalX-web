    ...
    <section className="py-20 md:py-28 bg-white" aria-labelledby="features-heading">
      <div className="max-w-[1400px] mx-auto px-5 md:px-16">
        <FadeUp className="mb-12">
          <span className="text-label-caps text-primary uppercase tracking-widest">What We Do</span>
          <h2
            id="features-heading"
        ...
        >
          {FEATURES.map((feature) => (
            <FadeUpChild key={feature.title}>
              <Card hover padding="lg" className="flex flex-col gap-4 h-full">
                <div className="w-11 h-11 rounded-sm bg-primary/8 flex items-center justify-center flex-shrink-0">
                  {feature.icon}
                </div>
                <h3 className="text-display-md text-ink">{feature.title}</h3>
                <p className="text-body-sm text-body-text leading-relaxed flex-1">{feature.desc}</p>
              </Card>
            </FadeUpChild>
          ))}
        </StaggerParent>
